...
#process tribal  !!
tribal_30 = aggregate_tribal(current_spi_30)
tribal_60 = aggregate_tribal(current_spi_60)
tribal_90 = aggregate_tribal(current_spi_90)
tribal_180 = aggregate_tribal(current_spi_180)
tribal_365 = aggregate_tribal(current_spi_365)
...
pal_bins <- colorBin(colorRamp(c("#8b0000", "#ff0000", "#ffff00", "#ffffff", "#00ffff", "#0000ff", "#000d66"), interpolate = "spline"), 
                     domain = -2.51:2.51, bins = seq(-2.51,2.51,0.5))
...
pal <- colorNumeric(c("#8b0000", "#ff0000", "#ffff00", "#ffffff", "#00ffff", "#0000ff", "#000d66"), -2.5:2.5, na.color = "transparent")
    ...
    m_tribal = m_tribal %>%
      addLayersControl(position = "topleft",
                       ...
                       overlayGroups = c("USDM", "States", "Weather"),
                       options = layersControlOptions(collapsed = FALSE)) %>%
      addLegend(pal = pal, values = -2.51:2.51,
                title = paste0("Current SPI<br>", as.character(watersheds_30$crrnt_t[1])),
                position = "bottomleft")
    
    saveWidget(m_tribal, "/home/zhoylman/drought_indicators/widgets/m_tribal_spi.html", selfcontained = F, libdir = "/home/zhoylman/drought_indicators/widgets/libs/")
