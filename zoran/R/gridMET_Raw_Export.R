...
write.dir = "/mnt/ScratchDrive/data/Hoylman/gridMET_Climatology/gridMET_precip_raw/"
...
foreach(i=1:length(time)) %dopar% {
  library(raster)
  writeRaster(gridMET[[i]], paste0(write.dir,"gridMET_", as.character(time_char_short[i]),"_precip.tif"), format = "GTiff", overwrite = T)
}
